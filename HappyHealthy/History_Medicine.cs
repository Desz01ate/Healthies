...
namespace HappyHealthyCSharp
{
    ...
    public class History_Medicine : ListActivity
    {
        ...
        {
            medList = pillTable.SelectAll(x => x.ud_id == this.GetPreference("ud_id", 0)).ToJavaList();//pillTable.GetJavaList<MedicineTABLE>($"SELECT * FROM MedicineTABLE WHERE UD_ID = {Extension.getPreference("ud_id", 0, this)}",new MedicineTABLE().Column);
